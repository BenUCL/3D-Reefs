...
def write_cameras_txt(output_path, camera_data, model_name):
    ...
    with open(output_path, 'w') as f:
        ...
        params_str = ' '.join(f'{p}' for p in camera_data['params'])
        f.write(f"{camera_data['camera_id']} {model_name} {camera_data['width']} {camera_data['height']} {params_str}\n")
...
def main():
    ...
    cameras_txt_backup = sparse_dir / 'cameras.txt.opencv_backup'
    ...
    if not cameras_bin_backup.exists():
        ...
        print(f"✓ Backed up cameras.bin to: {cameras_bin_backup.name}")
    else:
        print(f"✓ Backup already exists: {cameras_bin_backup.name}")
    
    # Backup cameras.txt if it exists
    if cameras_txt.exists() and not cameras_txt_backup.exists():
        shutil.copy(cameras_txt, cameras_txt_backup)
        print(f"✓ Backed up cameras.txt to: {cameras_txt_backup.name}")
    elif cameras_txt.exists():
        print(f"✓ Backup already exists: {cameras_txt_backup.name}")
    ...
    write_cameras_txt(cameras_txt, camera, 'PINHOLE')
    ...
    print()
    print(f"✓ Updated: {cameras_bin.name}")
    print(f"✓ Updated: {cameras_txt.name}")
    print()
    ...
    print(f"  cp {cameras_txt_backup} {cameras_txt}")
    print()
