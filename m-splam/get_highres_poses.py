...
def parse_colmap_images_txt(images_txt):
    ...
    while i < len(lines):
        ...
        # Extract timestamp from filename (e.g., "0.20000000298023224.png" → 0.2)
...
def write_colmap_images_txt_original(output_path, images_data):
    ...
    print(f"✓ Wrote {len(images_data)} images to {output_path}")
...
def write_full_mapping(output_path, images_data):
    ...
    """
    with open(output_path, 'w') as f:
        f.write("# Extended keyframe mapping: timestamp → frame_id → filenames\n")
    ...
    print(f"✓ Wrote extended mapping to {output_path}")
